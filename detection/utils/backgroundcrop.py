...
@njit
def get_box(image: np.ndarray, bar = 75, pad = 10, side = 150):
    xmin, ymin, xmax, ymax = 1e+5, 1e+5, 0, 0

    #임시로 하단 상태바, 측면 제거
    end_y = 1080 - bar
    end_x = 1920 - side
    image = image[:end_y,side:end_x,:]
    ...
    return xmin + side, ymin, xmax + side, ymax
...
def transform_all(image, bboxs, masks, target_size = 512):
    xmin, ymin, xmax, ymax = map(int, get_box(image))
...
if __name__ == '__main__':
    ...
    anno_info, image_info, origin = read_json(os.path.join(rootdir, 'coco_astrophysics.json'))
    ...
    for imgdir in tqdm(dirlist):
        ...
        re_img = Image.fromarray(cropped_img)
        re_img.save(imgdir)

    origin = edit_json(origin['images'], origin['annotations'], origin['categories'], img_list)
    #쓰기
    print('start writing')
    with open(os.path.join(rootdir, 'coco_astrophysics_dummy.json'), 'w', encoding='UTF-8') as json_file:

...
@njit
def get_box(image: np.ndarray, pad = 20):
    ...
    tmp[-1] = image.shape[-2] - tmp[-1] - 1 + pad
    xmin, xmax = tmp
    ...
    return xmin,  0, xmax, image.shape[0] - 1
...
if __name__ == '__main__':
    rootdir = r"D:\\resized\\test"
    dirlist = list(map(str, list(Path(rootdir).rglob('*.png'))))
    anno_info, image_info, origin = read_json(os.path.join(rootdir, 'modified_eval_dummy.json'))

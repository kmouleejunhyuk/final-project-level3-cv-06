...
BACK_IP = CONFIG.backend_ip
BACK_PORT = CONFIG.backend_port
...
def main():
    ...
    if model_radio == "Multi-label Classification":
        model_response = requests.get(BACK_IP + ":" + BACK_PORT + "/multilabel/model")
        st.sidebar.info(model_response.json())
    elif model_radio == "Object Detection":
        model_response = requests.get(BACK_IP + ":" + BACK_PORT + "/detection/model")
        st.sidebar.info(model_response.json())
    ...
    with placeholder.container(): # 6개 st line
        ...
        member_images = ['jiwoo.png', 'jiyoon.png', 'jiyou.jpeg', 'naeun.jpeg', 'jaehwan.jpeg', 'junhyeok.png', 'kyoungjae.png']
        member_route = [name_route+member for member in member_images]
        st.image(member_route, width=100, caption=["jiwoo", "jiyoon", "jiyou", "naeun", "jaehwan", "junhyeok", "kyoungjae"])
    ...
    if mode_radio == "New Image":
        ...
        if uploaded_file:
            ...
            if model_radio == 'Multi-label Classification':
                ...
                with placeholder.container():
                    ...
                    cls_response = requests.post(BACK_IP + ":" + BACK_PORT + "/multilabel/pred/", files=files)
                ...
                with placeholder.container():
                    ...
                    detect_response = requests.post(BACK_IP + ":" + BACK_PORT + "/detection/pred/", files=files)
        ...
        if model_radio == 'Multi-label Classification':
            file_response = requests.get(BACK_IP + ":" + BACK_PORT + "/multilabel/pred/")
            file_select = st.sidebar.selectbox("Images", file_response.json()) # key
            
            if file_select != 'None':
                result = requests.get(BACK_IP + ":" + BACK_PORT + f"/multilabel/pred/{file_select}")
                placeholder.empty()
                with placeholder.container():
                    st.markdown("____")
            ...
            file_response = requests.get(BACK_IP + ":" + BACK_PORT + "/detection/pred/")
            file_select = st.sidebar.selectbox("Images", file_response.json())

            if file_select != 'None':
                result = requests.get(BACK_IP + ":" + BACK_PORT + f"/detection/pred/{file_select}")
                placeholder.empty()
                with placeholder.container():
                    st.markdown("____")
                    result_img = Image.open(result.json()[0])
